...
/**
 * EditStudent Page — Pre-filled form to update an existing student.
 *
 * Flow:
 *   1. Extract student ID from URL params (:id)
 *   2. Fetch existing student data from GET /api/students/{id}
 *   3. Pre-fill the form with fetched data
 *   4. On submit → PUT /api/students/{id}
 */
function EditStudent() {
    ...
    const fetchStudent = async () => {
      try {
        ...
        setFormData({
          ...
          // Convert LocalDate array or string to YYYY-MM-DD for input[type=date]
          dateOfBirth: student.dateOfBirth
            ? Array.isArray(student.dateOfBirth)
              ? `${student.dateOfBirth[0]}-${String(student.dateOfBirth[1]).padStart(2,'0')}-${String(student.dateOfBirth[2]).padStart(2,'0')}`
              : student.dateOfBirth
            : '',
        });
      } catch (err) {
        setApiError('Could not load student data. ' + err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to student details after successful update
      navigate(`/students/${id}`, { state: { message: 'Student updated successfully!' } });
    } catch (err) {
